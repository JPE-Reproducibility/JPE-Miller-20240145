...
program main
    ...
    keep if log_IK != .
    create_stacked_did log_IK combo_dem_start

    reghdfe log_IK ib3._d_shifted##i.treated if min_obs <= -3 & max_obs >= 3 , absorb(i time) cluster(country_id year)

    reghdfe log_IK ib3._d_shifted##i.treated if min_obs <= -3 & max_obs >= 3 , absorb(i time#e_regionpol) cluster(country_id year)

    reghdfe log_IK ib3._d_shifted##i.treated if min_obs <= -3 & max_obs >= 3 , absorb(i time#region#L_autocracy) cluster(country_id year)

    reghdfe log_IK ib3._d_shifted##i.treated $EC $CC if min_obs <= -3 & max_obs >= 3 , absorb(i time#region#L_autocracy) cluster(country_id year)

end

program create_stacked_did
    args outcome treatment

    tempfile original
    save `original'

    levelsof year if `treatment' == 1, local(eventyear)
    foreach eventyr of local eventyear {
        
        use `original', clear
        
        * Include yet-to-be-treated as controls, in addition to never treated
        gen dummy = (year == `eventyr') & (`treatment' == 1)
        ...
        keep country_id year _t_exit cohort treated `outcome' $EC $CC e_regionpol region L_autocracy
