/*
Purpose: Generate Table C15 - Controlling for capital gains
Author: Max Miller
Date: 12-11-2025
*/
...
global CC log_ggdc_gdppc_i log_ggdc_gdppc_i_g all_eq_capgain
...
merge 1:1 country year using "${ASSETS_DERIVED}/all_equity_returns.dta", keep(1 3) nogen

winsor2 all_eq_capgain, replace

keep if year >= 1939 & year <= 1983	
winsor2 capm_unexp2, replace

gen treatment = .
label var treatment "Maj. Catholic Autocracy $\times$ Post"
...
esttab   
    using "${TABLES}/table_C15_did_capital_gains.tex", transform(@*100 100) b(%9.2f) se(%9.2f) 
